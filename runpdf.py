...
repo_path = os.path.join(current_directory, "Dafman")
...
'''
For loading pdf files we need to do the following,
1. pip install pypdf
2. add import -> from langchain_community.document_loaders import PyPDFLoader
3. add the loader to the file -
    loader = PyPDFLoader(filepath)
    pages = loader.load_and_split()
'''
# Load documents
loader = DirectoryLoader(repo_path, loader_cls = PyMuPDFLoader)
...
if not documents:
    ...
    print(f"Loaded {len(documents)} documents.")
    # Split documents
    pdf_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=1)
